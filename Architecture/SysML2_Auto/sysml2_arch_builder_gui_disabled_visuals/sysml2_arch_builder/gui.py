...
class ArchitectureBuilderWindow(QMainWindow):
    def __init__(self):
        ...
        self.subtype_combo.currentTextChanged.connect(self.update_allowed_option_checkboxes)
        self.signal_template_combo.currentTextChanged.connect(self.update_allowed_option_checkboxes)
        self.interface_template_combo.currentTextChanged.connect(self.update_allowed_option_checkboxes)
    ...
    def _build_menu(self):
        ...
        howto_action = QAction("HowTo / Documentation", self)
        howto_action.setStatusTip("Open the short SysML2 Architecture Builder HowTo PDF")
        howto_action.triggered.connect(self.open_howto)
        ...
        about_action.triggered.connect(lambda: QMessageBox.information(
            self,
            "About",
            "SysML2 Architecture Builder + Bus Content Wizard\n\n"
            "Architecture-first design helper for domains, architecture elements, ports, nets, bus contents and draw.io export."
        ))
        help_menu.addAction(about_action)

    def open_howto(self):
        candidates = [
            Path(__file__).resolve().parent / "docs" / "HowTo.pdf",
            Path(__file__).resolve().parents[1] / "docs" / "HowTo.pdf",
        ]
        for pdf_path in candidates:
            if pdf_path.exists():
                QDesktopServices.openUrl(QUrl.fromLocalFile(str(pdf_path)))
                return
        QMessageBox.warning(self, "HowTo missing", "HowTo.pdf was not found in the docs folder.")
    ...
    def set_widget_enabled_visual(self, widget, enabled: bool, reason: str = ""):
        """
        Enable/disable a widget and make the disabled state visually obvious.
        This avoids 'invisible logic' in the wizard: disabled fields are greyed out
        and do not look like active naming inputs.
        """
        ...
        if reason:
            widget.setToolTip(reason + "\n\nDisabled for the currently selected Net / Bus family.")
        else:
            widget.setToolTip("Disabled for the currently selected Net / Bus family.")


    def update_allowed_option_checkboxes(self):
        """Enable only GUI fields that are meaningful for the selected wizard family."""
        family = self.net_family_combo.currentText()

        rules = {
            "Interface": {
                "template": True, "signal": False, "domain": False, "subtype": False, "config": False,
                "additional": False, "voltage": False, "active_low": False, "differential": False,
                "power_out": False, "propagate": True, "bidi_side": True, "analog_side": False,
                "link_or_port": True, "start": True, "amount": True,
            },
            "Memory": {
                "template": True, "signal": False, "domain": True, "subtype": True, "config": True,
                "additional": True, "voltage": False, "active_low": False, "differential": False,
                "power_out": False, "propagate": True, "bidi_side": True, "analog_side": False,
                "link_or_port": False, "start": False, "amount": True,
            },
            "Control": {
                "template": True, "signal": True, "domain": False, "subtype": False, "config": False,
                "additional": True, "voltage": True, "active_low": True, "differential": False,
                "power_out": False, "propagate": True, "bidi_side": False, "analog_side": False,
                "link_or_port": False, "start": False, "amount": False,
            },
            "Status": {
                "template": True, "signal": True, "domain": False, "subtype": False, "config": False,
                "additional": True, "voltage": True, "active_low": True, "differential": False,
            ...
            "Analog": {
                "template": True, "signal": True, "domain": True, "subtype": False, "config": False,
                "additional": True, "voltage": True, "active_low": False, "differential": True,
                "power_out": False, "propagate": True, "bidi_side": False, "analog_side": True,
                "link_or_port": False, "start": False, "amount": False,
            },
            "Power": {
                "template": True, "signal": True, "domain": True, "subtype": False, "config": False,
                "additional": True, "voltage": True, "active_low": False, "differential": False,
                "power_out": True, "propagate": True, "bidi_side": False, "analog_side": False,
                "link_or_port": False, "start": False, "amount": False,
            },
        }
        ...
        widgets = {
            "template": self.interface_template_combo,
            "signal": self.signal_template_combo,
            "domain": self.domain_combo,
            "subtype": self.subtype_combo,
            "config": self.config_combo,
            "additional": self.additional_info_edit,
            "voltage": self.voltage_edit,
            "active_low": self.active_low_checkbox,
            "differential": self.differential_checkbox,
            "power_out": self.power_out_checkbox,
            "propagate": self.propagate_to_parent_checkbox,
            "bidi_side": self.bidirectional_side_combo,
            "analog_side": self.analog_side_combo,
            "link_or_port": self.link_or_port_combo,
            "start": self.interface_start_spin,
            "amount": self.interface_amount_spin,
        }

        explanations = {
            "template": "Primary bus/interface/template selection for the current family.",
            "signal": "Only required for single net families: Control, Status, Analog and Power.",
            "domain": "Used when the selected family needs an explicit technology/domain token.",
            "subtype": "Only used for Memory technology sub-selection.",
            "config": "Only used for Memory configuration.",
            "additional": "Optional device/source/sink/free naming token, where applicable.",
            "voltage": "Voltage or frequency token. Used for Power, Control, Status and Analog.",
            "active_low": "Only valid for Control and Status signals. Active low adds '#'.",
            "differential": "Only valid for Analog wizard signals where P/N variants can be generated.",
            "power_out": "Only valid for Power signals to mark the selected voltage as output.",
            "propagate": "Bottom-up export/visibility option for showing signals on parent/domain pages.",
            "bidi_side": "Only relevant for bidirectional Interface and Memory ports.",
            "analog_side": "Only relevant for Analog ports.",
            "link_or_port": "Only used for Interface templates containing [Link/Port].",
            "start": "Only used when creating multiple Interface instances.",
            "amount": "Only used for Interface and Memory batch creation.",
        }

        for key, widget in widgets.items():
            enabled = bool(r.get(key, False))
            # Empty combo boxes are also disabled to avoid confusing the user.
            if isinstance(widget, QComboBox) and widget.count() == 0:
                enabled = False
            self.set_widget_enabled_visual(widget, enabled, explanations.get(key, ""))

        # Reset invalid checkbox states immediately.
        if not r.get("active_low", False):
            self.active_low_checkbox.setChecked(False)
        if not r.get("differential", False):
            self.differential_checkbox.setChecked(False)
        elif family == "Analog" and not self.differential_checkbox.isChecked():
            self.differential_checkbox.setChecked(True)
        if not r.get("power_out", False):
            self.power_out_checkbox.setChecked(False)

        # Disabled fields should not suggest that they influence naming.
        if not r.get("link_or_port", False):
            self.link_or_port_combo.setCurrentText("Link")
        if not r.get("start", False):
