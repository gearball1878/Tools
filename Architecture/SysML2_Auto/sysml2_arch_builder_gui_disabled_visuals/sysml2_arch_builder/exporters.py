...
def write_pretty_xml(element: ET.Element, path: str | Path) -> Path:
    """
    Write XML in a readable, indented format.

    diagrams.net/draw.io accepts pretty-printed XML. This keeps generated .drawio
    files reviewable in Notepad++, git diffs, and code reviews.
    """
    p = Path(path)
    try:
...
class DrawioExporter:
    ...
    def save(self, path: str | Path) -> Path:
        ...
        p = Path(path)
        return write_pretty_xml(mxfile, p)
    ...
    def _emit_block(self, root: ET.Element, block, path: str, x: int, y: int) -> None:
        ...
        # diagrams.net page link: wrap the block mxCell in a UserObject.
        # This mirrors the manually verified draw.io XML format.
        if block.level == ArchitectureLevel.DOMAIN:
            user_attrs = {
                "id": cid,
                "label": label,
                "link": self._page_link(f"02_{block.name}"),
                "targetPage": f"02_{block.name}",
                "targetPageId": self._page_id(f"02_{block.name}"),
            }
        else:
            user_attrs = {
                "id": cid,
        ...
        user_obj = ET.SubElement(root, "UserObject", user_attrs)
        cell = ET.SubElement(user_obj, "mxCell", cell_attrs)
