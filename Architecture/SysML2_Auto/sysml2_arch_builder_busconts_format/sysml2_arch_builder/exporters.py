...
class BusContentsIniExporter:
    ...
    Export busconts.ini in the required plain format:
        ...
        <single_signal_group> <signal_name>

    No INI sections, no equals signs. Filename must be busconts.ini.
    """
    def __init__(self, model: ArchitectureModel):
        self.model = model
    ...
    def export_text(self) -> str:
        ...
        for ref in self.model.net_references:
            ...
            if signals:
                rhs = ",".join(str(s) for s in signals)
                lhs = ref.attributes.get("busContentName") or ref.name
            else:
                rhs = ref.name
