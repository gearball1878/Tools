...
class Port:
    ...
    def fqid(self, block_id: str) -> str:
        return f"{block_id}.{self.id}"
...
class Block:
    ...
    def find_port(self, block_id: str, port_id: str) -> Port:
        block = self.find_block(block_id)
        for port in block.ports:
            if port.id == port_id:
                return port
        raise KeyError(f"Port not found: {block_id}.{port_id}")
...
class Connection:
    ...
    name: str
    type: ConnectionType
    source: str  # format: BLOCK_ID.PORT_ID
    target: str  # format: BLOCK_ID.PORT_ID
    attributes: dict[str, Any] = field(default_factory=dict)
