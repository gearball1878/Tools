...
class GraphicModel(TransformModel):
    ...
    # For normal lines, w/h are the end-point delta. For Mentor arc/quadratic
    # objects, ctrl_x/ctrl_y store the control point relative to x/y in grid units.
...
class SymbolUnitModel:
    ...
    # Mentor records that are deliberately not interpreted yet, e.g. Q/GRPHSTL/FNTSTL.
    # TVRNT is intentionally not stored because only the 0° master representation is used.
    mentor_raw_unknown: List[str]=field(default_factory=list)
