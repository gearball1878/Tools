...
# ---------------------------------------------------------------------------
# Liebherr legacy patch bootstrap
# ---------------------------------------------------------------------------
# The original project accumulated a large set of versioned hotfix functions
# directly inside main_window.py.  To keep the GUI entry point maintainable
# without changing runtime behavior, the patch block is executed in this
# module's global namespace.  That preserves the original monkey-patch
# semantics and all existing globals/imports.
from pathlib import Path as _SW_Path

def _apply_main_window_legacy_patches():
    _patch_path = _SW_Path(__file__).with_name('_main_window_legacy_patches.py')
    _code = _patch_path.read_text(encoding='utf-8')
    exec(compile(_code, str(_patch_path), 'exec'), globals(), globals())

_apply_main_window_legacy_patches()

del _apply_main_window_legacy_patches
try:
    del _SW_Path
except Exception:
    pass
