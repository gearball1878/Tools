...
class MainWindow(QMainWindow):
    ...
    def _doc_markdown_path(self, filename: str) -> Path:
        """Return the canonical Markdown documentation path inside the package."""
        return Path(__file__).resolve().parents[1] / 'docs' / filename

    def _load_markdown_doc(self, filename: str, fallback_title: str) -> str:
        """Load UI documentation from symbol_wizard/docs.

        The Markdown file is the single source of truth for the corresponding
        Help dialog. Keep user-facing documentation out of Python string blobs.
        """
        path = self._doc_markdown_path(filename)
        try:
            return path.read_text(encoding='utf-8')
        except Exception as exc:
            return f'# {fallback_title}\n\nDocumentation file not found or unreadable:\n\n`{path}`\n\n{exc}'

    def _class_model_markdown(self):
        return self._load_markdown_doc('class_model.md', 'Class Model')
    ...
    def _how_to_markdown(self):
        return self._load_markdown_doc('how_to.md', 'Symbol Wizard - How To Guide')
...
# ---------------------------------------------------------------------------
# Ported from Symbol_Wizard_96: Pin transformation + loose docking only
# ---------------------------------------------------------------------------
# ---------------------------------------------------------------------------
