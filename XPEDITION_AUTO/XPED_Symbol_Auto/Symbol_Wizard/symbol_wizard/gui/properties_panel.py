...
from symbol_wizard.graphics.items import PinItem, SymbolBodyItem, TextGraphicsItem, qcolor_to_rgb, rgb_to_qcolor
from symbol_wizard.models.document import PinSide, PinType, SymbolBodyModel
...
class PropertiesPanel(QWidget):
    ...
    def _pin(self, item: PinItem):
        ...
        for label, attr in [
            ("Show Number", "visible_number"),
            ("Show Name", "visible_name"),
            ("Show Function", "visible_function"),
        ]:
            cb = QCheckBox()
    ...
    def _body(self, item: SymbolBodyItem):
        ...
        for attr_name in list(model.attributes.keys()):
            ...
            line.textChanged.connect(lambda v, a=attr_name: self._set_body_attribute(model, a, v))
            self.layout.addRow(attr_name, line)
