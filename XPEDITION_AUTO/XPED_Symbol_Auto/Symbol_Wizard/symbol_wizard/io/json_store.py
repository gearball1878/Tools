...
def _symbol(d):
    ...
    return SymbolModel(name=d.get('name','Symbol'), kind=kind, is_split=(kind==SymbolKind.SPLIT.value), grid_inch=d.get('grid_inch',0.1), sheet_format=d.get('sheet_format', SheetFormat.A3.value), origin=d.get('origin', OriginMode.CENTER.value), template_name=d.get('template_name',''), pin_palette=d.get('pin_palette') or SymbolModel().pin_palette, units=[_unit(x) for x in d.get('units',[]) ] or [SymbolUnitModel()])
