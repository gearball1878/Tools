...
def _symbol(d):
    ...
    return SymbolModel(name=d.get('name','Symbol'), kind=kind, is_split=(kind==SymbolKind.SPLIT.value), grid_inch=d.get('grid_inch',0.1), sheet_format=d.get('sheet_format', SheetFormat.A3.value), symbol_type=d.get('symbol_type','IC'), symbol_subtype=d.get('symbol_subtype','Generic IC'), origin=d.get('origin', OriginMode.CENTER.value), units=[_unit(x) for x in d.get('units',[]) ] or [SymbolUnitModel()])
