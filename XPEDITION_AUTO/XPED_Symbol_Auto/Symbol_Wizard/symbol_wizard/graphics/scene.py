...
class SymbolScene(QGraphicsScene):
    ...
    @property
    def grid_px(self):
        # Editor snap/display grid.  The Template Editor may use a finer
        # edit grid (0.050" / 0.025") while the symbol itself keeps its
        # logical/base grid, usually 0.100".
        try:
            edit_grid = getattr(self.window, 'edit_grid_inch', None)
            if edit_grid:
                return float(edit_grid) * PX_PER_INCH
        except Exception:
            pass
