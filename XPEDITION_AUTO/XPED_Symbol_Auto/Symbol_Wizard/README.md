...
The Template Editor loads symbol types from `Symbol_Wizard/symbol_wizard/symbol_types/symbol_types.json`.
Stage 1 intentionally ships only resistor templates.

## Mentor 0° Master Graphics Import Update

The Mentor/Xpedition importer now keeps visible 0° master geometry instead of reducing symbols to body/pin rectangles.

Supported visible primitives:
- `l` Mentor line/polyline records are imported as editable line segments.
- `b` Mentor box records are imported as the editable body for the first box and as graphic rectangles for additional boxes.
- `c` Mentor circle records are imported as ellipse/circle graphics.
- `a` Mentor arc records are imported as editable curved line graphics with preserved start/control/end geometry.
- `T`, `U`, `P`, `L`, and `A` remain supported as text, symbol attributes, pins, labels, and pin attributes.

Deliberate behavior:
- `|TVRNT` orientation variants are ignored on import. Only the 0° master representation is used.
- Style/helper records such as `Q`, `|GRPHSTL`, `|GRPHSTL_EXT01`, `|FNTSTL`, and border metadata are retained as raw metadata for future refinement where possible.
- Export can regenerate visible primitives from the Wizard JSON; orientation variants can be added later by the librarian or a dedicated export enhancement.
